...
from app.default_inbound import (
    DEFAULT_INBOUND_TAG,
    is_default_inbound_enabled,
)
...
class UserCreate(User):
    ...
    def validate_inbounds(cls, inbounds, values, **kwargs):
        ...
        # When a single Default Inbound is enforced, every protocol is bound to
        # it regardless of what the client submitted.
        if is_default_inbound_enabled():
            for proxy_type in proxies:
                inbounds[proxy_type] = [DEFAULT_INBOUND_TAG]
            return inbounds
...
class UserModify(User):
    ...
    @field_validator("inbounds", mode="before")
    def validate_inbounds(cls, inbounds, values, **kwargs):
        # When a single Default Inbound is enforced the inbound is fixed; ignore
        # whatever the client submitted and keep everything bound to DEFAULT.
        if is_default_inbound_enabled():
            proxies = values.data.get("proxies", {}) or {}
            return {proxy_type: [DEFAULT_INBOUND_TAG] for proxy_type in proxies}
