...
from app.default_inbound import is_default_inbound_enabled
...
class User(Base):
    ...
    def inbounds(self):
        if is_default_inbound_enabled():
            # Single permanent inbound: every proxy is bound to DEFAULT.
            _ = {}
            for proxy in self.proxies:
                _[proxy.type] = [DEFAULT_INBOUND_TAG]
            return _
...
class System(Base):
    ...
    # Persisted Reality keypair + short id for the single Default Inbound.
    # Stored here so keys survive restarts / rebuilds (see app/default_inbound.py).
    reality_settings = Column(JSON, nullable=True)
