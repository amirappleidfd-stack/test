...
def set_reality_settings(db: Session, settings: dict) -> None:
    """Persist the Default Inbound Reality keypair + short id.

    Stored on the singleton ``system`` row so keys survive restarts / rebuilds.
    """
    from app.db.models import System as SystemModel
    row = db.query(SystemModel).filter(SystemModel.id == 1).first()
    if not row:
        row = SystemModel(id=1)
        db.add(row)
    row.reality_settings = settings
    db.commit()


def get_reality_settings(db: Session) -> Optional[dict]:
    """Load persisted Reality settings for the Default Inbound, or None."""
    from app.db.models import System as SystemModel
    row = db.query(SystemModel).filter(SystemModel.id == 1).first()
    if row and row.reality_settings:
        return row.reality_settings
    return None
