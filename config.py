...
# ---------------------------------------------------------------------------
# Default Inbound (single fixed VLESS + Reality + XHTTP inbound)
# When DEFAULT_INBOUND is true the panel enforces a single, permanent inbound.
# All users are attached to it automatically, new inbounds cannot be created,
# and the inbound is restored on every (re)start. Reality keys survive restarts
# via the database (or via the explicit XRAY_REALITY_*_KEY override env vars).
# ---------------------------------------------------------------------------
DEFAULT_INBOUND = config("DEFAULT_INBOUND", default=False, cast=bool)
DEFAULT_INBOUND_TYPE = config("DEFAULT_INBOUND_TYPE", default="vless")
DEFAULT_NETWORK = config("DEFAULT_NETWORK", default="xhttp")
DEFAULT_SECURITY = config("DEFAULT_SECURITY", default="reality")

# External (public) endpoint shown to clients in subscription links.
# Xray itself never binds to this host; it only listens on XRAY_INTERNAL_PORT.
XRAY_EXTERNAL_HOST = config("XRAY_EXTERNAL_HOST", default="")
XRAY_EXTERNAL_PORT = config("XRAY_EXTERNAL_PORT", default=0, cast=int)

# Internal port Xray actually listens on (0.0.0.0:XRAY_INTERNAL_PORT).
XRAY_INTERNAL_PORT = config("XRAY_INTERNAL_PORT", default=8080, cast=int)

# The upstream site Reality pretends to be (used as SNI / serverNames).
TARGET_DOMAIN = config("TARGET_DOMAIN", default="")

# Optional explicit Reality keypair + short id. When set they take precedence
# over any value stored in the database and survive container rebuilds. Leave
# empty to have the panel generate and persist them on first run.
XRAY_REALITY_PRIVATE_KEY = config("XRAY_REALITY_PRIVATE_KEY", default="")
XRAY_REALITY_PUBLIC_KEY = config("XRAY_REALITY_PUBLIC_KEY", default="")
XRAY_REALITY_SHORT_ID = config("XRAY_REALITY_SHORT_ID", default="")
